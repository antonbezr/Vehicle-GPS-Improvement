...
The Arduino Uno was developed so that it would output sensor measurements any time there was a new reading from each sesnor. This data was then transferred to the Raspberry Pi 4 via serial port. Afterward, the sensor measurements were combined using a Kalman filter to determine position. After calculation, the computed GPS coordinate was sent to the OLED display.
